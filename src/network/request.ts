import { authWhiteList, getCommonParams } from '@/config/index'
import accountStore from '@/store/account'
import { Response, Dictionay } from '@/types/common'
...
const conbineOptions = (_opts: RequestOptions): AxiosRequestConfig => {
  ...
  let commonHeaders: Dictionay = {}
  if (!authWhiteList.includes(opts.url || '')) {
    const { userId, token } = accountStore
    if (userId && token) {
      commonHeaders = {
        'Client-Token': token,
        'Client-Uid': userId,
